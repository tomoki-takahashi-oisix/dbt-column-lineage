...
class DbtSqlglot:
    ...
    def recursive(self, base_source: str, next_source: str, base_column: str, next_columns: [], depth: int) -> None:
        ...
        # dbt_compiled_path = dbt_node.get('compiled_path')
        ...
        for after_base_column, after_next_sources_columns in after_next_sources_columns_dict.items():
            ...
            for after_next_source in after_next_sources:
                ...
                if self.request_depth != -1 and depth > self.request_depth:
                    continue
                # self.logger.info(f'base_source={after_base_source}, next_source={after_next_source}, base_column={after_base_column}, next_columns={after_next_columns}, depth={depth}')
                self.recursive(after_base_source, after_next_source, after_base_column, after_next_columns, depth)
        if not next_found:
            # 再起の最後だったらedgeの起点がない
            self.nodes[-1]['data']['last'] = True
    ...
    def add_edge(self, base_column: str, columns: [], base_source: str, source: str):
        if not base_source or len(columns) == 0:
            self.logger.info(f'{base_source} is None or {columns} len = 0')
            return
        ...
        for column in columns:
            self.logger.info(f'source={base_source}, target={source.lower()}, base_column={base_column}, column={column}')
            edge_id = f'{base_edge_id}-{base_column}-{column}'
            if self.find(self.edges, 'id', edge_id):
                self.logger.info(f'already exist edge: {edge_id}')
                continue
            else:
                self.edges.append({
                    'id': edge_id,
    ...
    def get_sqlglot_lineage(self, source: str, compiled_code: str, columns: [], depends_on_table_info: []) -> (dict, bool):
        ...
        # sqlglot_db_schema = MappingSchema(dialect='snowflake', normalize=False)
        sqlglot_db_schema = {}
        ...
        for column in columns:
            ...
            for node in lin.walk():
                if isinstance(node.expression, exp.Table):
                    # node_name = node.name.replace(f'{node.expression.alias}.', '')
                    # if node_name != '*' and node_name != column:
                    #     self.logger.info(f'column change: {column} => {node_name}')
                    #     column = node_name
                    # if not self.replace_columns:
                    # self.replace_columns = list(columns)
                    # self.replace_columns.append(node_name)
        ...
        # self.logger.info(compiled_path, ret)
        return ret, found

    def str_to_base_10_int_str(self, s:str):
        return str(int(s, 36))
