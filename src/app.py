...
@app.route(f'{BASE_ROUTE}/folder_dashboards', methods=['GET'])
def list_folder_dashboards():
    if USE_OAUTH and session.get('access_token') is None:
        return jsonify({'error': 'not authorized'}), 401

    looker = Looker(logger)
    folder_dashboards = looker.get_folder_dashboards()
    return jsonify(folder_dashboards), 200


@app.route(f'{BASE_ROUTE}/dashboard_elements/<dashboard_id>', methods=['GET'])
def get_dashboard_elements(dashboard_id):
    if USE_OAUTH and session.get('access_token') is None:
        return jsonify({'error': 'not authorized'}), 401

    looker = Looker(logger)
    dashboard_elements = looker.get_dashboard_elements(dashboard_id)
    return jsonify(dashboard_elements), 200


@app.route(f'{BASE_ROUTE}/explore_fields/<slug>', methods=['GET'])
def get_explore_fields(slug):
    if USE_OAUTH and session.get('access_token') is None:
        return jsonify({'error': 'not authorized'}), 401

    looker = Looker(logger)
    explore_fields = looker.get_explore_fields(slug)
    return jsonify(explore_fields), 200
